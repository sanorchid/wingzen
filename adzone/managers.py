...
class AdManager(models.Manager):
    ...
    def get_ad_by_title(self, ad_title, ad_zone, ad_category=None):
        """
        Returns an advert by id that belongs for the specified ``ad_category``
        and ``ad_zone``.
        If ``ad_category`` is None, the ad will be category independent.
        """
        qs = self.get_query_set().filter(title=ad_title)
        if ad_category:
            qs = qs.filter(category__slug=ad_category)
        try:
            ad = qs[0]
