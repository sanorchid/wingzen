...
def topic_publish(request, category_id=None):
    ...
    return render(request, 'spirit/topic/topic_publish.html', {'form':form})
...
def topics_index(request):
    topics = Topic.objects.for_public().filter(is_pinned=False)
    topics = topics.order_by('category', '-last_active').select_related('category')
    ...
    latest_comments = Comment.objects.filter(is_removed=False).order_by('-date').select_related('topic')[0:15]
    ...
    return render(request, 'spirit/index.html', {'categories': categories,
                                                 'topics': panel_topics,
